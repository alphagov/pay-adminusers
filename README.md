...
| `NOTIFY_DIRECT_DEBIT_API_KEY`                                  | The GOV.UK Notify API key to use when sending direct debit messages. Defaults to `api_key-pay-notify-service-id-pay-notify-secret-needs-to-be-32-chars-fsghdngfhmhfkrgsfs`. |
| `NOTIFY_FORGOTTEN_PASSWORD_EMAIL_TEMPLATE_ID`                  | The GOV.UK Notify template ID to use for sending password reset emails. Defaults to `pay-notify-forgotten-password-email-template-id`. |
| `NOTIFY_INVITE_SERVICE_EMAIL_TEMPLATE_ID`                      | A GOV.UK Notify template ID. Defaults to `pay-notify-invite-service-email-template-id`. |
| `NOTIFY_INVITE_SERVICE_USER_DISABLED_EMAIL_TEMPLATE_ID`        | A GOV.UK Notify template ID. Defaults to `pay-notify-invite-service-user-disabled-email-template-id`. |
| `NOTIFY_INVITE_SERVICE_USER_EXITS_EMAIL_TEMPLATE_ID`           | A GOV.UK Notify template ID. Defaults to `pay-notify-invite-service-user-exists-email-template-id`. |
| `NOTIFY_INVITE_USER_EMAIL_TEMPLATE_ID`                         | A GOV.UK Notify template ID. Defaults to `pay-notify-invite-user-email-template-id`. |
| `NOTIFY_INVITE_USER_EXISTING_EMAIL_TEMPLATE_ID`                | A GOV.UK Notify template ID. Defaults to `pay-notify-invite-user-existing-email-template-id`. |
| `NOTIFY_LIVE_ACCOUNT_CREATED_EMAIL_TEMPLATE_ID`                | A GOV.UK Notify template ID. Defaults to `pay-notify-live-account-created-email-template-id`. |
| `NOTIFY_MANDATE_CANCELLED_EMAIL_TEMPLATE_ID`                   | A GOV.UK Notify template ID. Defaults to `pay-mandate-cancelled-email-template-id`. |
| `NOTIFY_MANDATE_FAILED_EMAIL_TEMPLATE_ID`                      | A GOV.UK Notify template ID. Defaults to `pay-mandate-failed-email-template-id`. |
| `NOTIFY_ONE_OFF_MANDATE_AND_PAYMENT_CREATED_EMAIL_TEMPLATE_ID` | A GOV.UK Notify template ID. Defaults to `pay-one-off-mandate-and-payment-created-email-template-id`. |
| `NOTIFY_ON_DEMAND_MANDATE_CREATED_EMAIL_TEMPLATE_ID`           | A GOV.UK Notify template ID. Defaults to `pay-on-demand-mandate-created-email-template-id`. |
| `NOTIFY_ON_DEMAND_PAYMENT_CONFIRMED_EMAIL_TEMPLATE_ID`         | A GOV.UK Notify template ID. Defaults to `pay-on-demand-payment-confirmed-email-template-id`. |
| `NOTIFY_PAYMENT_FAILED_EMAIL_TEMPLATE_ID`                      | A GOV.UK Notify template ID. Defaults to `pay-payment-failed-email-template-id`. |
